...
mod test {
    ...
    fn t_topological_sorting_0() {
        ...
        let roots = sorter.run_from_range(&mut nodes, 1..2).unwrap();
        assert_eq!(roots, 2..3);
    ...
    fn t_topological_sorting_1() {
        ...
        let roots = sorter.run_from_range(&mut nodes, 4..5).unwrap();
        assert_eq!(roots, 5..6);
        assert_eq!(
            nodes,
            vec![
    ...
    fn t_topological_sorting_2() {
        ...
        let roots = sorter.run_from_range(&mut nodes, 10..11).unwrap();
    ...
    fn t_prune_cyclic() {
        ...
        assert!(matches!(
            sorter.run_from_range(&mut nodes, 2..3),
            Err(Error::CyclicGraph)
        ));
    }
}
